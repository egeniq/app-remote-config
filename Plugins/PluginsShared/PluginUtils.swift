...
enum PluginUtils {
    private static var supportedConfigFiles: Set<String> {
        Set(["yaml", "yml"].map { "app-remote-config-settings." + $0 })
    }
    private static var supportedDocFiles: Set<String> { Set(["yaml", "yml", "json"].map { "app-remote-config." + $0 }) }
    ...
    static func validateInputs(
        ...
        let arguments = [
            "generate", "\(doc)", "--settings", "\(config)", "--output-directory", "\(genSourcesDir)", "--plugin-source",
            "\(pluginSource.rawValue)",
        ]

        let tool = try tool("care")
